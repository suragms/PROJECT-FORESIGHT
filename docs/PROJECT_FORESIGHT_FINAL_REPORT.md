...
**Author:** Surag M S  
**Status:** PROJECT DELIVERY READY WITH DOCUMENTED LIMITATIONS  
**Production Model:** phase20_synthetic_lightgbm

> **Dataset note:** Synthetic production data is locally generated (`src/generate_synthetic_retail.py`), not the official Kaggle 10M download. See `docs/phase24_dataset_provenance_matrix.md`.
...
| Dataset | Role | Grain | Provenance |
|---------|------|-------|------------|
| UCI Online Retail II | Research Candidate | Invoice-day | Pipeline CSV verified; Kaggle ZIP not archived |
| Synthetic Retail (local) | Production Source | Weekly SKU | Generated by `generate_synthetic_retail.py`; Kaggle 10M not used |
